...
# STOLEN FROM https://github.com/fmu2/flow-VAE/blob/master/static_flow_vae.py
...
class PlanarFlow(nn.Module):
    def __init__(self, latent_dimension, device=None, dtype=None):
        super(PlanarFlow, self).__init__()
        factory_kwargs = {'device': device, 'dtype': dtype}
        self.latent_dimension = latent_dimension
        
        self.w = nn.Parameter(torch.empty((1, latent_dimension), **factory_kwargs))
        self.b = nn.Parameter(torch.empty((1,1), **factory_kwargs))
        self.u = nn.Parameter(torch.empty((1, latent_dimension), **factory_kwargs))
    ...
    def forward(self, x):
        ...
        # these steps are needed to normalize parameters and gradient issues
...
class HouseholderFlow(nn.Module):
    ...
    def forward(self, x):
        ...
        v_sqr = self.v.norm()**2
        H = torch.eye(self.d) - 2. * outer / v_sqr
        x = torch.mm(H, x.t()).t()
