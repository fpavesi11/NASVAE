...
"""
SECOND VERSION OF VAE LOSS FOR IAF
"""

class VAELossV2(_Loss):
    def __init__(self, reduction='sum', cat_loss_weight=1, feat_loss_weight=1, 
    ...
    def forward(self, y, y_true, z, eps, mu, log_var, log_det):
        ...
        mseloss = CustomReconLosslinear(reduction='none')
        features_loss, category_loss = mseloss(y, y_true)
        reconstructon_loss = features_loss * self.feat_loss_weight + self.cat_loss_weight * category_loss
        ...
        overall_loss = reconstructon_loss + passage_kl_weight * KL_div
